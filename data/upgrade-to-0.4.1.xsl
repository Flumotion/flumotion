...
<xsl:output method="xml" indent="yes" />
  ...
  <xsl:template match="//planet/*/component/@version">
    <xsl:attribute name="version">0.4.1</xsl:attribute>
  </xsl:template>
  ...
  <xsl:template match="//planet/*/component/@type">
    <xsl:attribute name="type">
      <xsl:choose>
        ...
        <xsl:when test=". = 'icalbouncer'"
          >ical-bouncer</xsl:when>
        <xsl:when test=". = 'tokentestbouncer'"
          >tokentest-bouncer</xsl:when>
        ...
        <!-- The following components aren't in flumotion core, but are in
             public svn, so this is ok -->
        <xsl:when test=". = 'dvb'"
          >dvb-producer</xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="." />
        </xsl:otherwise>
      </xsl:choose>
