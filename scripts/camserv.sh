...
python $FLUPATH/producer.py -v -c localhost -n producer -p "v4lsrc device=/dev/video0 ! video/x-raw-yuv,width=320,height=240"  > /tmp/flumotion.producer.log 2>&1 &
sleep 3
xterm -T producer -e sh -c "tail -n 10000 -f /tmp/flumotion.producer.log" &

echo Starting converter
python $FLUPATH/converter.py -v -c localhost -s producer -n converter -p "ffmpegcolorspace ! jpegenc ! multipartmux"  > /tmp/flumotion.converter.log 2>&1 &
sleep 3
