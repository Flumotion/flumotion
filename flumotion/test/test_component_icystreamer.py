...
class TestIcyStreamer(comptest.CompTestTestCase, log.Loggable):
    ...
    def testReceivingTagEvent(self):
        ...
        d.addCallback(lambda _: comptest.delayed_d(0.1, 'some title'))
        d.addCallback(assertTitleSet)

        d.addCallback(lambda _: self._sendTitleEvent('some other title'))
        d.addCallback(lambda _: comptest.delayed_d(0.1, 'some other title'))
        d.addCallback(assertTitleSet)

        # also check non-ascii title case
        title = 'za\xc5\xbc\xc3\xb3\xc5\x82\xc4\x87'
        d.addCallback(lambda _: self._sendTitleEvent(title))
        d.addCallback(lambda _: comptest.delayed_d(0.1, title))
        d.addCallback(assertTitleSet)
