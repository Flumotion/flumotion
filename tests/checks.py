...
def make_model():
    ...
    for s in data:
        i = m.append()
        m.set_value(i, 0, s)
        m.set_value(i, 1, lambda: eval('video.%s'%s,{'video':video}))
    return m
    ...
    def run_check(self):
        ...
        def callback(res, d):
            if d != self.current_deferred:
                print '(got successful old result: %s->%r)' % (name, res)
            else:
                print '%s successful: %r' % (name, res)

        def errback(res, d):
            if d != self.current_deferred:
                print '(got failing old result: %s->%r)' % (name, res)
            else:
                print '%s failed, reason: %r' % (name, res)
